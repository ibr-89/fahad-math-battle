...
$("#shareRoomBtn").addEventListener("click", async ()=>{
  ...
  const text=`انضم إلى Math Race | سباق الرياضيات\nPIN: ${activeRoomPin}\n${url}`;
  if(navigator.share){
    try{await navigator.share({title:"Math Race | سباق الرياضيات",text,url});return;}catch(_){}
  }
...
function podiumCard(player, place, totalQuestions){
  ...
  card.setAttribute("aria-label",`${place===1?"المركز الأول":place===2?"المركز الثاني":"المركز الثالث"}: ${player?.name||"لاعب"}`);

  const medal=document.createElement("div");
  medal.className="podium-medal";
  medal.textContent=ar(place);

  const crown=place===1?document.createElement("div"):null;
  if(crown){
  ...
  const champion=place===1?document.createElement("div"):null;
  if(champion){
    champion.className="champion-ribbon";
    champion.textContent="★ بطل المسابقة ★";
  }

  card.append(medal);
  if(crown)card.append(crown);
  card.append(avatar,placeLabel);
  if(champion)card.append(champion);
  card.append(name,score,correct);
  return card;
}
