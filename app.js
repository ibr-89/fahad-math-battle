...
async function startGameAsHost() {
  ...
  Object.keys(room.players||{}).forEach(pid=>{
    ...
    scoreReset[`players/${pid}/answers`]=null;
  });
...
function runHostTimer(room){
  ...
  const tick=()=>{
    ...
    const all=Object.keys(roundAnswers(room,idx)).length>=Object.keys(room.players||{}).length && Object.keys(room.players||{}).length>0;
    if(remain<=0 || all){
...
function podiumCard(player, place, totalQuestions){
  ...
  medal.textContent=place===1?"🥇":place===2?"🥈":"🥉";
  ...
  card.append(medal,placeLabel,name,score,correct);
  return card;
}
